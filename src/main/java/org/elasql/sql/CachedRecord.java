...
public class CachedRecord implements Record, Serializable {
	...
	public static CachedRecord newRecordForInsertion(RecordKey key,
		...
		CachedRecordBuilder builder = new CachedRecordBuilder(key, true, false);
		for (Map.Entry<String, Constant> entry : fldVals.entrySet())
			builder.addFldVal(entry.getKey(), entry.getValue());
	...
	private CachedRecord(RecordKey primaryKey) {
...
//	}
	...
	CachedRecord(RecordKey primaryKey, Map<String, Constant> nonKeyFldVals,
			boolean isNewInserted, boolean isTemp) {
		this.primaryKey = primaryKey;
		this.nonKeyFldVals = nonKeyFldVals;
